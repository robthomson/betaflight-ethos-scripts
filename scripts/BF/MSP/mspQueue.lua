...
function MspQueueController:processQueue()
    ...
    local cmd, buf, err
    --bf.print("retryCount: "..self.retryCount)

    if not bf.runningInSimulator then
        if not self.lastTimeCommandSent or self.lastTimeCommandSent + 0.8 < bf.clock() then
            if self.currentMessage.payload then
                --bf.print("Sending  cmd "..self.currentMessage.command..": {" .. joinTableItems(self.currentMessage.payload, ", ") .. "}")
                bf.protocol.mspWrite(self.currentMessage.command, self.currentMessage.payload)
            else
                --bf.print("Sending  cmd "..self.currentMessage.command)
                bf.protocol.mspWrite(self.currentMessage.command, {})
            end
            self.lastTimeCommandSent = bf.clock()
        ...
        if not self.currentMessage.simulatorResponse then
            bf.print("No simulator response for command "..tostring(self.currentMessage.command))
            self.currentMessage = nil
            return
        end
    ...
    if cmd then
        self.lastTimeCommandSent = nil
        bf.print("Received cmd: "..tostring(cmd))
    end
    if err then bf.print("  ERROR flag set!") end
    ...
    elseif self.retryCount > self.maxRetries then
        bf.print("Max retries reached, aborting queue")
...
function MspQueueController:add(message)
    ...
    bf.print("Queueing command "..message.command.." at position "..#self.messageQueue + 1)
