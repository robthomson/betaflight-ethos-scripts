...
function mspProcessTxQ()
    ...
    -- if not sensor:idle() then  -- was protocol.push() -- maybe sensor:idle()  here??
        -- bf.print("Sensor not idle... waiting to send cmd: "..tostring(mspLastReq))
        -- return true
    -- end
    --bf.print("Sending mspTxBuf size "..tostring(#mspTxBuf).." at Idx "..tostring(mspTxIdx).." for cmd: "..tostring(mspLastReq))
    local payload = {}
    ...
    local i = 2
    while (i <= bf.protocol.maxTxBufferSize) and mspTxIdx <= #mspTxBuf do
        payload[i] = mspTxBuf[mspTxIdx]
        mspTxIdx = mspTxIdx + 1
        mspTxCRC = bit32.bxor(mspTxCRC,payload[i])
        i = i + 1
    end
    if i <= bf.protocol.maxTxBufferSize then
        payload[i] = mspTxCRC
        i = i + 1
        -- zero fill
        while i <= bf.protocol.maxTxBufferSize do
            payload[i] = 0
            i = i + 1
        end
        bf.protocol.mspSend(payload)
        mspTxBuf = {}
        mspTxIdx = 1
        mspTxCRC = 0
        return false
...
function mspSendRequest(cmd, payload)
    --bf.print("Sending cmd "..cmd)
    -- busy
    if #(mspTxBuf) ~= 0 or not cmd then
        bf.print("Existing mspTxBuf is still being sent, failed send of cmd: "..tostring(cmd))
        return nil
    end
    mspTxBuf[1] = #(payload)
...
local function mspReceivedReply(payload)
    --bf.print("Starting mspReceivedReply")
    local idx = 1
    ...
    idx = idx + 1
    --bf.print("payload length: "..#payload)
    --bf.print(" msp sequence #:  "..string.format("%u",seq))
    if start then
        -- start flag set
        mspRxBuf = {}
        ...
        if mspRxReq == mspLastReq then
            mspStarted = true
            --bf.print("Started cmd "..mspLastReq)
        end
    elseif not mspStarted then
		bf.print("  mspReceivedReply: missing Start flag")
        return nil
    elseif bit32.band(mspRemoteSeq + 1, 0x0F) ~= seq then
        mspStarted = false
        return nil
    end
    while (idx <= bf.protocol.maxRxBufferSize) and (#mspRxBuf < mspRxSize) do
        mspRxBuf[#mspRxBuf + 1] = payload[idx]
        mspRxCRC = bit32.bxor(mspRxCRC, payload[idx])
        idx = idx + 1
    end
    if idx > bf.protocol.maxRxBufferSize then
		--bf.print("  mspReceivedReply:  payload continues into next frame.")
    ...
    if mspRxCRC ~= payload[idx] and version == 0 then
		bf.print("  mspReceivedReply:  payload checksum incorrect, message failed!")
        --bf.print("    Calculated mspRxCRC:  0x"..string.format("%X", mspRxCRC))
        --bf.print("    CRC from payload:     0x"..string.format("%X", payload[idx]))
        return nil
    end
    --bf.print("  Got reply for cmd "..mspRxReq)
    return true
end

function mspPollReply()
    local startTime = bf.clock()
    while (bf.clock() - startTime < 0.05) do
        local mspData = bf.protocol.mspPoll()
